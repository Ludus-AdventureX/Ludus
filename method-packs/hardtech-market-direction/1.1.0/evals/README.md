...
## 1.1.0 新增 gate 与工程合同验收

- `cynefin-gate-routes.json` 检查五类领域分流、chaotic/disorder 默认阻断和人类 override；
- 所有现有 eval 的 recommended method version 指向 1.1.0；
- 安装器还必须编译 `source-span`、`judgment-set`、`dissent-record`、`draft-recommendation`、`validator-result`、`validator-aggregate` 和 `deep-analysis-result`，并拒绝缺失或额外 Validator。
