...
本目录是 `hardtech-market-direction@1.1.0` 的定稿源版本。`release_candidate` 表示内容已经完成方法评审、等待安装器校验和内容哈希发布；它不表示运行时可直接读取 `ways`。
...
本包共编译 17 个 source skill、10 个 Prompt 和 17 个 Draft 2020-12 schema；新增 Cynefin、SourceSpan、Judgment/Dissent、DeepAnalysisResult 与 V1-V9 Validator 合同。执行合同以 `manifest.yaml`、`diagnostic-questions.yaml`、`quality-gates.yaml`、`prompts/` 和 `schemas/` 为准。
...
正式运行前必须执行 `cynefin-gate-result`：clear 默认 quick，complicated 默认 focused，complex 允许 focused/full，chaotic 先稳定且默认阻断长分析，disorder 阻断并要求补充边界。任何 override 只能由人类提交并冻结到 Charter/RunManifest。

### 结构化 Agent Engine

正式输入是 Case Charter + 冻结材料快照 + 分析深度 + 方法/预算/工具 envelope；正式输出是 ID/hash-only `DeepAnalysisResult`：Run Manifest、JudgmentSet、DissentRecord、DraftRecommendation、Unknown、九项 ValidatorResult、quality gate 与 provenance 都先持久化，再通过 Workspace/Run-scoped API 读取。不是 `messages[]` 聊天循环，也不内嵌第二套领域 DTO。

### 责任和发布边界

本方法只产生 `analysis` responsibility 的产物。它不能签署 DecisionRecord，也不能把建议标记为 Human。没有 qualifying Run、没有 SourceSpan 或存在 validation blocker 时，不得发布报告或产生可签署决定。
