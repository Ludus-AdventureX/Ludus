...
## Migration discipline — DEFERRAL RELEASED (chained after Task 10)
This lane was code-first with the Alembic revision withheld per the charter's ordering (my `down_revision` must point at Task 10's migration). A1 has now reported its rev-id: **`b6e8f3a1d7c2`** (`add_analysis_outputs`, chained after the frozen head `f9a4b7e2c8d3`). The lane revision **`a7c3e9f1b5d8_add_dossier_version_snapshots.py`** is therefore generated from `app/dossiers/models.py` metadata with `down_revision = 'b6e8f3a1d7c2'` and lands last, exactly as declared. Two facts for integrators/QA:
- The parent revision FILE ships on the A1 branch (`codex/task-10-quality-gate-r1`), not on this branch. In this worktree the lane revision has a dangling parent **by design**; `alembic upgrade head` becomes applicable only after the integration merge unites both branches. The full upgrade→check→downgrade→re-upgrade lifecycle pass is integration-wave-owned and must run on the merged chain (expected single post-merge head: `a7c3e9f1b5d8`).
- QA-7 (`test_qa7_lane_migration_chained_to_task10_revision`) is re-pinned to the released state: exact 10-file `versions/` set, chain wiring `a7c3e9f1b5d8 -> b6e8f3a1d7c2`, and the expected in-worktree head pair `{f9a4b7e2c8d3, a7c3e9f1b5d8}` (collapses to a single head after the merge). It fails loudly on any silent chain movement.
Owner/QA tests continue to materialise the companion table via `Base.metadata.create_all(checkfirst=True)` on the migrated test database, which stays valid pre- and post-merge.
