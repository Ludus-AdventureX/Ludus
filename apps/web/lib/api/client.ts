...
/**
 * Double-submit CSRF wiring. AGENTS.md requires every cookie-authenticated
 * mutation to carry an Origin/Referer-checked double-submit token. The cookie
 * and header names below are the assumed defaults; the canonical names are an
 * open INTEGRATION_QUESTION for the Case/API/Data owner until the auth routes
 * land, and are centralised here so a single edit realigns the whole client.
 */
export const CSRF_COOKIE_NAME = "ludus_csrf";
 ...
 * or an empty body. The error `body` is intentionally left as `unknown`: the
 * canonical error-envelope schema is not yet part of the generated contract, so
 * callers must not assume its shape (tracked as an INTEGRATION_QUESTION).
 */
